    ...
    <div className="space-y-6">
      <Card>
        <div className="flex items-center justify-between">
          <div>
            <h3 className="font-semibold flex items-center gap-2"><Bot className="h-4 w-4 text-indigo-500" /> {t("globalSettings.agentArchitecture", "Arquitectura de agentes")}</h3>
            <p className="mt-1 text-sm text-muted-foreground">
              {scope === "global"
                ? t("globalSettings.agentGlobalDesc", "Preferencias por defecto del workspace. Se aplican como base al crear nuevos proyectos.")
                : t("globalSettings.agentProjectDesc", "Configuración específica de este proyecto. Sobreescribe las preferencias globales.")}
            </p>
          </div>
          {agents.configured ? <Badge tone="green">{t("globalSettings.configured", "Configurados")}</Badge> : <Badge tone="amber">{t("globalSettings.unconfigured", "Sin configurar")}</Badge>}
        </div>
      </Card>

      <div>
        <div className="mb-2 px-1 text-[11px] font-semibold uppercase tracking-wider text-slate-400">{t("globalSettings.transversal", "Transversal")}</div>
        <Card>
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="font-medium text-sm">{t("globalSettings.clarifierAgent", "Agente Clarificador")}</div>
              <div className="text-xs text-muted-foreground">{t("globalSettings.clarifierDesc", "Refina el prompt inicial del usuario antes de pasarlo a cada etapa.")}</div>
            </div>
            <button
          ...
          <div key={s.key}>
            <div className={`mb-2 flex items-center gap-2 rounded-lg border ${c.border} ${c.bg} px-3 py-2`}>
              ...
              <div className="flex items-center gap-2">
                <span className={`text-[10px] font-bold tabular-nums rounded-full px-1.5 py-0.5 bg-white/70 ${c.text}`}>{t("globalSettings.stage", "Etapa")} {idx + 1}</span>
                <span className={`text-[12px] font-semibold ${c.text}`}>{t(`stages.${s.key}.label`, s.label)}</span>
                <span className="text-[11px] text-muted-foreground">· {t(`stages.${s.key}.sub`, s.sub)}</span>
              </div>
            </div>
            <Card className={`border-l-4 ${c.border}`}>
              <div className="grid gap-4 md:grid-cols-2">
                <AgentRow label={t("globalSettings.creator", "Creador")} desc={t("globalSettings.creatorDesc", "Genera el contenido de la etapa.")} spec={agents[s.key].creator} keys={keys} onChange={(p) => updateStage(s.key, "creator", p)} onOpenApiKeys={onOpenApiKeys} onEditPrompt={() => setEditing(`${s.key}.creator` as AgentSlotKey)} />
                <AgentRow label={t("globalSettings.reviewer", "Revisor")} desc={t("globalSettings.reviewerDesc", "Valida y refina el output.")} spec={agents[s.key].reviewer} keys={keys} onChange={(p) => updateStage(s.key, "reviewer", p)} onOpenApiKeys={onOpenApiKeys} onEditPrompt={() => setEditing(`${s.key}.reviewer` as AgentSlotKey)} />
              </div>
            </Card>
          </div>
        );
